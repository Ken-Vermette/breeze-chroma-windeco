...
namespace Breeze
{
    ...
    QColor Decoration::titleBarColor() const
    {
        if( m_animation->state() == QPropertyAnimation::Running )
          ...
          return m_colorSettings.titleBar( client().data()->isActive() );
    }

    //________________________________________________________________
    QColor Decoration::titleBarHighlightColor() const
    {
        if( m_animation->state() == QPropertyAnimation::Running )
          return KColorUtils::mix( m_colorSettings.inactiveTitleBar(), m_colorSettings.activeTitleBar(), m_opacity )
            .lighter(client().data()->isActive() ? 120 : 100);
        else
          return titleBarColor().darker(titleBarColor().value() > 128 ? 120 : 100);
    }

    //________________________________________________________________
    QColor Decoration::titleBarOutlineColor() const
    {
        // If this is an animation, mix colours.
        QColor outlineColor = QColor ( m_animation->state() == QPropertyAnimation::Running ?
            KColorUtils::mix( m_colorSettings.inactiveTitleBar(), m_colorSettings.activeTitleBar(), m_opacity ) :
            m_colorSettings.titleBar( client().data()->isActive() )
        );

        //100 is the baseline, values under make it lighter
        return outlineColor.darker( outlineColor.value() < 128 ? 130 : 90 );
    }
    ...
    QColor Decoration::outlineColor() const
    {
        ...
        else if (!client().data()->isActive() && titleBarColor() == m_colorSettings.palette().color( QPalette::Window ))
            return QColor();
        else
          return m_colorSettings.palette().color( QPalette::Highlight );
    }
    ...
    void Decoration::init()
    {
        ...
        connect(client().data(), &KDecoration2::DecoratedClient::maximizedChanged, this, &Decoration::updateSizeGrip);
        connect(client().data(), &KDecoration2::DecoratedClient::activeChanged,    this, &Decoration::updateSizeGrip);
    ...
    //________________________________________________________________
    void Decoration::updateSizeGrip()
    {
        if( hasNoBorders() && m_internalSettings->drawSizeGrip() && !isMaximized() )
          createSizeGrip();
        else
          deleteSizeGrip();
    }
    ...
    void Decoration::paintTitleBar(QPainter *painter, const QRect &repaintRegion)
    {
        ...
        gradient.setColorAt(0, titleBarColor.lighter( client().data()->isActive() ? 110 : 100 ));
        ...
        if (isMaximized()) {
            ...
            painter->setClipRect(titleRect, Qt::IntersectClip); // make the rect a taller to clip away the rounded corners on bottom
        ...
        if( !c->isShaded() && outlineColor.isValid() ) {
            ...
            // Contrst Line
            painter->setPen( QPen(  titleBarOutlineColor(), 1  ) );
            painter->drawLine( QPointF(0,borderTop() - 3), QPointF(size().width(), borderTop() - 3) );
        }
